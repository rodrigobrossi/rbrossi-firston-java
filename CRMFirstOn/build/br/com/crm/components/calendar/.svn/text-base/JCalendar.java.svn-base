...
public class JCalendar extends JPanel {
	...
	private static final long serialVersionUID = -2347240055697778136L;
	...
	public static class BasicDate implements Serializable {
		private static final long serialVersionUID = 1039727399658185232L;
	...
	private Color colorDayHighlightBackground = new Color(33,55,44);
	...
	private MouseMotionListener mouseMotionListener = new MouseMotionListener() {
		...
		public void mouseMoved(MouseEvent e) {
			beginClickDate = null;
			if (isEnabled()) {
				long oldDate = cursorDate == null ? 0 : cursorDate.getDate()
						.getTime();
				Date mouseOverDate = getDate(e.getX(), e.getY());
				long newDate = mouseOverDate == null ? 0 : mouseOverDate
						.getTime();
				if (oldDate != newDate) {
					cursorDate = new BasicDate(mouseOverDate);
					ActionEvent event = new ActionEvent(JCalendar.this,
							ActionEvent.ACTION_PERFORMED, ACTION_CURSOR_MOVED,
	...
	private MouseListener mouseListener = new MouseListener() {
		...
		public void mouseReleased(MouseEvent e) {
			if (isEnabled() && beginClickDate != null) {
				Date mouseOverDate = getDate(e.getX(), e.getY());
				if (mouseOverDate != null
						&& beginClickDate.equals(new BasicDate(mouseOverDate))) {
					ActionEvent event = new ActionEvent(JCalendar.this,
							ActionEvent.ACTION_PERFORMED, ACTION_CLICKED,
							System.currentTimeMillis(), 0);
					fireActionPerformed(event);
				}
				beginClickDate = null;
	 ...
	 * 
	 * @param useRolloverHighlight
	 *            if true, rollover highlighting is enabled.
	 */
	 ...
	 * 
	 * @param dateX
	 *            the x coordinate used in the lookup
	 * @param dateY
	 *            the y coordinate used in the lookup
			...
			for (int iColumn = 0; iColumn < 8; iColumn++) {
				...
				if (iRow == 0 || iColumn == 0) {
					...
					if (iColumn > 0) {
						...
						Rectangle2D dayBounds = fontMetrics.getStringBounds(
								day, g);
						int dayX = (int) (x + (cellWidth / 2) - (dayBounds
								.getWidth() / 2));
					...
					BasicDate dateObject = new BasicDate(new Date(date
							.getTimeInMillis()));
					long oldDate = dateObject == null ? 0 : dateObject
							.getDate().getTime();
					long newDate = cursorDate == null ? 0 : cursorDate
							.getDate().getTime();
	 ...
	 * 
	 * @param date
	 *            the date used to set the month and year
	 */
	 ...
	 * @param color
	 *            the color to be used in highlighting the date cell
	 */
	 ...
	 * 
	 * @param date
	 *            the date for which the highlight color is desired
