 ...
 */
package br.com.crm.components.calendar;
...
public class JDateChooser extends JPanel {
	private static final long serialVersionUID = -2376604169318441820L;

	/** Stores the last date formatter that successfully parsed a date */
	protected static DateFormat lastDateFormat = new SimpleDateFormat(
			"MM/dd/yyyy");
	...
	protected static final SimpleDateFormat[] DATE_FORMATS = new SimpleDateFormat[] {
			new SimpleDateFormat("M/d/yyyy"), new SimpleDateFormat("M-d-yyyy"),
			...
			new SimpleDateFormat("dd MMMM,yyyy"),
			new SimpleDateFormat("dd MMMM yyyy"),
			new SimpleDateFormat("yyyy-M-d"), new SimpleDateFormat("yyyy/M/d"),
	...
	public JDateChooser() {
		...
		comboBoxButton = new BasicArrowButton(BasicArrowButton.SOUTH, UIManager
				.getColor("ComboBox.buttonBackground"), UIManager
				.getColor("ComboBox.buttonShadow"), UIManager
		...
		dateField.setPreferredSize(new Dimension(125, dateField
	...
	private Action onSelectDate = new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().equalsIgnoreCase("Clicked")) {
				hideDatePicker();
				ActionEvent event = new ActionEvent(JDateChooser.this,
						ActionEvent.ACTION_PERFORMED, "changed", System
								.currentTimeMillis(), 0);
	 ...
	 * 
	 * @param component
	 *            the component whose frame is needed
	 */
	...
	public void showDatePicker() {
		if (datePickerWindow == null) {
			datePickerWindow = new JWindow(getFrame(this.getParent()));
		...
		if (datePickerVisible == false) {
			Date date = getDate();
			if (date != null || datePicker.getDate() != null)
				datePicker.setDate(date);
			Dimension max = java.awt.Toolkit.getDefaultToolkit()
					.getScreenSize();
			int x = JDateChooser.this.getWidth() - 250
					+ JDateChooser.this.getLocationOnScreen().x;
			int y = JDateChooser.this.getLocationOnScreen().y
					+ JDateChooser.this.getHeight();
			int w = 250;
			int h = 220;
			if (x + w > max.width)
				x = max.width - w;
	...
	private class ComboBoxLayout implements LayoutManager {
		...
		public Dimension preferredLayoutSize(Container parent) {
			Dimension dimension = new Dimension(comboBoxButton
					.getPreferredSize().width
					+ dateField.getPreferredSize().width, combineDimensions(
					new Dimension[] { comboBoxButton.getPreferredSize(),
							dateField.getPreferredSize() }, true).height);
			return dimension;
		}
		...
		public void layoutContainer(Container parent) {
			...
			if (comboBoxButton != null) {
				comboBoxButton.setBounds(width - (insets.right + buttonSize),
	 ...
	 */
	private static Dimension combineDimensions(Dimension[] dimensions,
			boolean maximum) {
		if (maximum) {
			int w = 0;
			int h = 0;
	...
	private static boolean isInPopup(Component source) {
		for (Component component = source; component != null; component = component
				.getParent()) {
