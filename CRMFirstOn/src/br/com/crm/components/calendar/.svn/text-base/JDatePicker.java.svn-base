...
import br.com.crm.components.calendar.JCalendar.BasicDate;
...
public class JDatePicker extends JPanel {
	private static final long serialVersionUID = -824399887109524399L;
	 ...
	 */
	private static int[] DAYS_IN_MONTH_LIST = { 31, 28, 31, 30, 31, 30, 31, 31,
			30, 31, 30, 31 };
	...
	private Color colorSelectedRange = new Color(99,200,00);
	...
	public JDatePicker() {
		...
		calendarRegion.setBackground(Color.WHITE);
		calendarRegion.setBorder(BorderFactory.createRaisedBevelBorder());
		calendarRegion.setLayout(new GridBagLayout());
		calendarRegion.add(monthComboBox, new GridBagConstraints(0, 1, 1, 1,
				1.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(3, 3, 3, 3), 2, 2));
		calendarRegion.add(yearSpinner, new GridBagConstraints(1, 1, 1, 1, 1.0,
				0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(3, 3, 3, 3), 2, 2));
		calendarRegion.add(calendarComponent, new GridBagConstraints(0, 2, 2,
				1, 1.0, 1.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

		this.add(calendarRegion, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0,
	...
	private ActionListener calendarActions = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand() == JCalendar.ACTION_CURSOR_MOVED) {
				...
				updateStatus();
				updateCalendarHighlights();
				ActionEvent event = new ActionEvent(JDatePicker.this,
						ActionEvent.ACTION_PERFORMED, "clicked", System
								.currentTimeMillis(), 0);
	 ...
	 * 
	 * @param date
	 *            the date to be selected and highlighted in the UI
	 */
	public void setDate(Date date) {
		if (date != null) {
			...
			this.date = null;
			calendarComponent.setCalendarView(oldDate == null ? new Date()
					: oldDate);
		}
		updateView();
	}
