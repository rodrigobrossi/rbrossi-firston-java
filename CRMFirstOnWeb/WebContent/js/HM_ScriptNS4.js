/*HM_ScriptNS4.js
 * by Peter Belesis. v4.0.11 010530
...
HM_a_Parameters = [ [ "MenuWidth", 150, "number" ],
	...
	[ "BorderColor", "red" ], [ "SeparatorSize", 1, "number" ],
...
window.onresize = function() {
    if (window.innerWidth == HM_NS_OrigWidth
	    && window.innerHeight == HM_NS_OrigHeight)
	return;
    HM_f_Initialize();
...
function HM_f_MakeTrees() {
	...
	HM_CurrentTree = {
	    ...
	    HorizOffsetRight : HorizOffsetRight = (parseInt((HM_ChildPerCentOver != null) ? (HM_ChildPerCentOver * ItemWidth)
		    : HM_ChildOverlap))
		    - HM_ItemPadding,
	    HorizOffsetLeft : (MenuWidth - HorizOffsetRight) - (HM_BorderWidth)
		    - HM_ItemPadding,
	    FontColor : FontColor = HM_f_EvalParameters(TreeParams[3],
		    HM_FontColor),
	    FontColorOver : FontColorOver = HM_f_EvalParameters(TreeParams[4],
		    HM_FontColorOver),
	...
	if (HM_CurrentTree.TopIsPermanent) {
		...
		moveTo(eval(HM_CurrentTree.MenuLeft),
			eval(HM_CurrentTree.MenuTop));
		visibility = "show";
	    }
	}
    }
...
function HM_f_GetItemHtmlStr(arraystring) {
    ...
    if (HM_FontItalic)
	TempString = TempString.italics();
    TempString = "<FONT FACE='" + HM_FontFamily + "' POINT-SIZE=" + HM_FontSize
	    + ">" + TempString + "</FONT>";
...
function HM_f_MakeMenu(menucount) {
    ...
    if (!NewMenu) {
	eval(HM_MenuIDPrefix + menucount
		+ " = new Layer(HM_CurrentTree.MenuWidth,window)")
    ...
    if (!HM_CurrentTree.treeParent)
	HM_CurrentTree.treeParent = HM_CurrentTree.startChild = HM_CurrentMenu;

    while (HM_CurrentMenu.itemCount < HM_CurrentMenu.maxItems) {
	HM_CurrentMenu.itemCount++;
	HM_CurrentItem = eval("window." + HM_ItemIDPrefix + menucount + "_"
		+ HM_CurrentMenu.itemCount);
	if (!HM_CurrentItem) {
	    eval(HM_ItemIDPrefix
		    + menucount
		    + "_"
		    + HM_CurrentMenu.itemCount
		    + " = new Layer(HM_CurrentTree.ItemWidth - (HM_ItemPadding*2),HM_CurrentMenu)")
	    if (HM_StatusDisplayBuild)
		status = "Creating Hierarchical Menus: " + menucount + " / "
			+ HM_CurrentMenu.itemCount;
	    HM_CurrentItem = eval(HM_ItemIDPrefix + menucount + "_"
		    + HM_CurrentMenu.itemCount);
	    HM_CurrentItem
		    .itemSetup(menucount + "_" + HM_CurrentMenu.itemCount);
	}
	if (HM_CurrentItem.hasMore
	...
	HM_CurrentMenu.clip.right = HM_CurrentMenu.lastItem.left
		+ HM_CurrentMenu.lastItem.clip.right + HM_BorderWidth;
    } else {
	HM_CurrentMenu.clip.right = HM_CurrentTree.MenuWidth;
    }
    HM_CurrentMenu.clip.bottom = HM_CurrentMenu.lastItem.top
	    + HM_CurrentMenu.lastItem.clip.bottom + HM_BorderWidth;
...
function HM_f_ItemSetup(itemidsuffix) {
	...
	if (this.index)
	    this.left = this.siblingBelow.left + this.siblingBelow.clip.width
		    + HM_SeparatorSize;
	else
	...
	this.left = (HM_BorderWidth + HM_ItemPadding);
	if (this.index)
	    this.top = this.siblingBelow.top + this.siblingBelow.clip.height
		    + HM_SeparatorSize;
	else
	    this.top = (HM_BorderWidth + HM_ItemPadding)
    }
    ...
    this.txtLyrOff = new Layer(HM_CurrentTree.ItemTextWidth
	    - (HM_ItemPadding * 2), this);
    with (this.txtLyrOff) {
	document.write(this.permHilite ? this.htmStrOver : this.htmStr);
	document.close();
	if (HM_CurrentTree.RightToLeft && this.menu.showImage)
	    left = HM_ItemPadding + HM_ImageSize + HM_ImageHorizSpace;
	visibility = "inherit";
    }

    if (this.tree.NSFontOver) {
	if (!this.permHilite) {
	    this.txtLyrOn = new Layer(HM_CurrentTree.ItemTextWidth
		    - (HM_ItemPadding * 2), this);
	    with (this.txtLyrOn) {
		if (HM_CurrentTree.RightToLeft && this.menu.showImage)
		    left = HM_ItemPadding + HM_ImageSize + HM_ImageHorizSpace;
	...
	with (this.imgLyr) {
	    document.write("<IMG SRC='"
		    + (HM_CurrentTree.RightToLeft ? HM_ImageSrcLeft
			    : HM_ImageSrc) + "' WIDTH=" + HM_ImageSize
		    + " VSPACE=0 HSPACE=0 BORDER=0>");
	    document.close();
	    moveBelow(this.txtLyrOff);
	    left = (HM_CurrentTree.RightToLeft) ? HM_ImageHorizSpace
		    : this.tree.ItemWidth - (HM_ItemPadding * 2) - HM_ImageSize
			    - HM_ImageHorizSpace;
...
function HM_f_PopMenu(e) {
    ...
    HM_CurrentMenu.xPos = (HM_CurrentMenu.tree.MenuLeft != null) ? eval(HM_CurrentMenu.tree.MenuLeft)
	    : mouse_x_position;
    HM_CurrentMenu.yPos = (HM_CurrentMenu.tree.MenuTop != null) ? eval(HM_CurrentMenu.tree.MenuTop)
	    : mouse_y_position;
...
function HM_f_ShowChild() {
	...
	this.child.yPos = this.menu.top + this.menu.clip.height
		- HM_BorderWidth;
    } else {
