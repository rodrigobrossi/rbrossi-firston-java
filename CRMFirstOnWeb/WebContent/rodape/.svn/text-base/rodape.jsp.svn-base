...
<jsp:include page="validaParametro.jsp" flush="true"></jsp:include>

<%
	request.getSession();
	FOLocalizationBean trd2 = (FOLocalizationBean) session
	...
	rsCount = conexao.executaConsulta(query2, session.getId() + "RS1");
	int teste1 = 0;
	if (rsCount.next()) {
	...
	rsCount = conexao.executaConsulta(query2, session.getId() + "RS2");
	int teste2 = 0;
	if (rsCount.next()) {
	...
	rsCount = conexao.executaConsulta(query2, session.getId() + "RS3");
	int teste3 = 0;
	if (rsCount.next()) {
...
function array1(){
	...
	<%/*Processo de avaliação dos projetos*/
			...
			rsIdioma = conexao.executaConsulta(query, session.getId()
					+ "Idioma");
			if (rsIdioma.next()) {
...
<%/*Processo de avaliação dos projetos*/
			...
			rsPlano = conexao
					.executaConsulta(query3, session.getId() + "Plano");
			if (rsPlano.next()) {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<%
					if (apl.equals("EM")) {
				%>
				<td nowrap><a href="#" onclick="return sobre()" class="dilink"><%=trd2.Traduz("Sobre o")%>
				FirstOn EM</a></td>
				<%
		...
		%> <a href="#" onMouseOver="HM_f_PopUp('elMenu3',event)"
			...
			alt=<%=("\"" + trd2.Traduz("Alterar Idioma") + "\"")%>></a> <%
 	}
 ...
 %>
		...
		<td align="right" width="14"><a href="#"
			...
			width="29" height="20" border="0"
			alt=<%=("\"" + trd2.Traduz("Alterar PLANO") + "\"")%>></a></td>
		<td>&nbsp;</td>
		<%
			if (!usu_tipo.equals("F")) {
		%>
		<td align="right" width="14"><a href="#"
			...
			alt=<%=("\"" + trd2.Traduz("Alterar FILIAL") + "\"")%>></a></td>
		<%
			}
...
<%
	if (rsIdioma != null) {
		rsIdioma.close();
		conexao.finalizaConexao(session.getId() + "Idioma");
	}
	if (rsPlano != null) {
		rsPlano.close();
		conexao.finalizaConexao(session.getId() + "Plano");
	}
	if (rsFilial != null) {
		rsFilial.close();
		conexao.finalizaConexao(session.getId() + "Filial");
	}
	...
	//}
%>
<jsp:include page="acesso.jsp" flush="true"></jsp:include>
</html>
<script language="JavaScript1.2" src="../js/HM_LoaderP.js"
