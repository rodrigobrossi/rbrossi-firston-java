...
%>
<%@page import ="java.sql.ResultSet,java.util.Vector,java.text.SimpleDateFormat"%>
...
String query = "", query_grid = "", query_c = "", loc = "", vcampo = ""; 
...
"FILIAL.FIL_CODIGO =* FUNCIONARIO.FIL_CODIGO AND " +
...
if (request.getParameter("pa") != null){
  if(!(request.getParameter("pa").equals(""))) {
...
if (request.getParameter("ia") != null){ 
  if (!(request.getParameter("ia").equals(""))) { 
...
if (rsc.next()){
	tot = rsc.getInt(1);
        if (tot>0) existe = true;//caso nao exista registros
...
<head>
<title>FirstOn - <%=trd.Traduz("Registro")%> - <%=trd.Traduz("INCLUIR PARTICIPANTES NAO PLANEJADOS")%></title>
...
function Filtro(){
	window.open("10_turmaantecipada_part_inc.jsp?select2="+frm.select2.value+"&turma=<%=turma%>","_parent");
	return true;
}        	

function Unidade(){
	window.open("10_turmaantecipada_part_inc.jsp?op_uni="+frm.select_uni.value+"&turma="+frm.turma.value+"&turma=<%=turma%>","_parent");
	return true;
}    

function Diretoria(){
	window.open("10_turmaantecipada_part_inc.jsp?op_uni="+frm.select_uni.value+"&op_dir="+frm.select_dir.value+"&turma=<%=turma%>","_parent");
	return true;
}    
function Celula(){
	window.open("10_turmaantecipada_part_inc.jsp?op_uni="+frm.select_uni.value+"&op_dir="+frm.select_dir.value+"&op_cel="+frm.select_cel.value+"&turma=<%=turma%>","_parent");
	return true;
}    
function Time(){
	window.open("10_turmaantecipada_part_inc.jsp?op_uni="+frm.select_uni.value+"&op_dir="+frm.select_dir.value+"&op_cel="+frm.select_cel.value+"&op_tim="+frm.select_tim.value+"&turma=<%=turma%>","_parent");
	return true;
}    
...
function solicex()
	...
	frm.action = "10_turmaantecipada_part_inc_grava.jsp";
        frm.submit();
	return false;
        }
          ...
          <td width="100%" align="center"> 
            <table border="0" cellspacing="0" cellpadding="0">
              <tr> 
                ...
                <td class="trontrk" align="center"><%=trd.Traduz("INCLUIR PARTICIPANTES NAO PLANEJADOS")%></td>
...
<%                  }%>
              ...
              <tr align="center"><td class="ctfontc" colspan="9">&nbsp;</td></tr>
              <tr><td height="1" class="ctvdiv" colspan="9"><img src="../art/bit.gif" width="1" height="1"></td></tr>
              <tr><td height="12" colspan="9"><img src="../art/bit.gif" width="1" height="1"></td></tr>
              <tr align="center">
              <td class="ctfontc" valign="middle" colspan="9"> * - <%=trd.Traduz("TERCEIRO")%>
              </td>
              </tr>
              <tr><td height="12" colspan="9"><img src="../art/bit.gif" width="1" height="1"></td></tr>
              <tr><td height="1" class="ctvdiv" colspan="9"><img src="../art/bit.gif" width="1" height="1"></td></tr>
...
<%                    //Dados do curso escolhido
                      ...
                      rsTurma = conexao.executaConsulta(queryTurma,session.getId()+"RS_7");
                      if (rsTurma.next()) {
                        String dataf="", datai="";
...
<%                      if (funcvet.contains(rsgrid.getString(1))){%>                          
                            <td width="4%"><input type="checkbox" name="checkbox<%=i%>" value="<%=rsgrid.getString(1)%>" checked></td>
<%                          funcvet.remove(rsgrid.getString(1));
                        } else {%>
                            <td width="4%"><input type="checkbox" name="checkbox<%=i%>" value="<%=rsgrid.getString(1)%>"></td>
<%                      }%>
