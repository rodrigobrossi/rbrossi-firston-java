<!--
Nome do arquivo: cadastro/competencias.jsp
Nome da funcionalidade: cadastro de competencias
Função: exibe as competências e as funcionalidades, dependendo das permissões e 
...
<%
	...
	ResultSet rs = null;
	String filtro = "", query = "", imp_comp = "", valorFiltro = "";
	...
	if (!(request.getParameter("filtro") == null)) {
		filtro = " WHERE CMP_DESCRICAO >= '"
				+ request.getParameter("filtro") + "' ";
	}

	query = "SELECT CMP_CODIGO, CMP_DESCRICAO FROM COMPETENCIA "
			+ filtro + "ORDER BY CMP_DESCRICAO";
	imp_comp = prm.buscaparam("IMP_COMPETENCIA");
%>
...
<head>
<title>FirstOn - <%=trd.Traduz("Cadastro")%> - <%=trd.Traduz("Cadastro de CompetEncias")%></title>
<script language="JavaScript" src="/js/scripts.js"></script>
<script language="JavaScript">
function filtra(){
  document.frm.tipo.value = "F";
  frm.action ="competencias.jsp";
  frm.submit();
  return false; 
}
function inclui(){
      document.frm.tipo.value = "I";
  frm.action ="inclusaodecompet.jsp";
  frm.submit();
  return false; 
}
function altera(){
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<table border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="13"><img src="../art/bit.gif" width="13"
							height="15"></td>
						<td class="trontrk" width="297" align="center"><%=trd.Traduz("CADASTRO DE COMPETENCIAS")%></td>
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td height="20" class="ctfontc" align="center">&nbsp; <%=trd.Traduz("LOCALIZAR POR COMPETENCIA")%>:
						<%
 	valorFiltro = (((String) request.getParameter("filtro") == null) ? ""
 ...
 %>
					...
					<tr>
						...
						<%
							...
							if (per.contains("CADASTRO COMPETENCIA - MANUTENCAO")) {
								mostraCheck = true;
						%>
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
						...
						<table border="0" cellspacing="1" cellpadding="2" width="80%">
							<tr>
								...
								<%
									if (existe) {
								%>
								<td width="96%" class="celtittab"><%=trd.Traduz("NOME DO COMPETENCIA")%>
								<%
									} else {
								%>
