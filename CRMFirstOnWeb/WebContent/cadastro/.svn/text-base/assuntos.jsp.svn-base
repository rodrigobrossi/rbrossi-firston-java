<!--
Nome do arquivo: cadastro/assuntos.jsp
Nome da funcionalidade: cadastro de assunto
Função: exibe os assuntos e as funcionalidades para assunto
...
<%
	...
	ResultSet rs = null;
	String query = "", msg = "", filtro = "", valorFiltro = "";
	...
	if (!(request.getParameter("filtro") == null)) {
		filtro = " WHERE ASS_NOME >= '"
				+ request.getParameter("filtro") + "' ";
	}

	//Abre a conexAo com o Banco e faz o Resultset para mostrar os Assuntos
	query = "SELECT ASS_CODIGO, ASS_NOME, ASS_ATIVO, ASS_AVALIAEFICACIA FROM ASSUNTO "
			+ filtro + "ORDER BY ASS_NOME";
%>

<%@page import="firston.eval.components.FOLocalizationBean"%>
<%@page import="firston.eval.connection.FODBConnectionBean"%><html>
<head>
<title>FirstOn - <%=trd.Traduz("Cadastro")%> - <%=trd.Traduz("CADASTRO DE ASSUNTO")%></title>
<script language="JavaScript" src="/js/scripts.js"></script>
<script language="JavaScript">
function filtra(){
  document.frm.tipo.value = "F";
  frm.action ="assuntos.jsp";
  frm.submit();
  return false; 
}
function inclui(){
    document.frm.tipo.value = "I";
  frm.action ="inclusaodeassunto.jsp";
  frm.submit();
  return false; 
}
function altera(){
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<table border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="13"><img src="../art/bit.gif" width="13"
							height="15"></td>
						<td class="trontrk" align="center"><%=trd.Traduz("Cadastro de Assunto")%></td>
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td height="20" class="ctfontc" align="center">&nbsp; <%=trd.Traduz("LOCALIZAR POR ASSUNTO")%>:
						<%
 ...
 %>
					...
					<tr>
						...
						<%
							...
							if (per.contains("CADASTRO ASSUNTO - MANUTENCAO")) {
								mostraCheck = true;
						%>
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
						...
						<table border="0" cellspacing="1" cellpadding="2" width="80%">
							<tr>
								...
								<%
									if (existe) {
								%>
								<td width="96%" class="celtittab"><%=trd.Traduz("NOME DO ASSUNTO")%>
								<%
									} else {
								%>
...
<%
	...
	//}catch (Exception r){out.println("ERRO:"+r);}
%>