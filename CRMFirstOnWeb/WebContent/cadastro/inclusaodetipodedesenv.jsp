...
//Pega o tipo de cadastro (inclusAo ou alteraCAo) e o codigo do Tipo de Desenvolvimento
String tipo = "";
String cod = "1";
...
ResultSet rs = null;
String query = "SELECT DES_CODIGO, DES_DESCRICAO "+
         "FROM DESENVOLVIMENTO " + 
         "WHERE DES_CODIGO = " + cod + " ORDER BY DES_DESCRICAO";     
%>
...
if (tipo.equals("I")){
  %>
        <%=trd.Traduz("INCLUSAO DE ALTERNATIVA DE DESENVOLVIMENTO")%>
        <%
}
else{
  %>
  <%=trd.Traduz("ALTERACAO DE ALTERNATIVA DE DESENVOLVIMENTO")%>
  <%
}
%>
</title>
...
function envia(){
  if(document.frm.desnome.value==""){
    alert(<%=("\""+trd.Traduz("Digite a alternativa de desenvolvimento")+"\"")%>);
    return false;
  }
  else{
    frm.submit();
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
  <tr> 
    ...
    if (tipo.equals("I"))
    {
      ...
      <FORM name = "frm" action="tipodesenvgrava.jsp" method="POST">
          <td valign="top"> <img src="../art/bit.gif" width="159" height="1">&nbsp;<br><center>
                <table border="0" cellspacing="2" cellpadding="3" width="90%">
                  <tr class="celnortab"> 
                    <td>        
                      <p><%=trd.Traduz("ALTERNATIVA DE DESENVOLVIMENTO")%><br>
                      <%
      if (tipo.equals("I")){
        String desnome = (((String)request.getParameter("desnome")==null)?"":(String)request.getParameter("desnome"));  
        %> <input type="text" name="desnome" maxlength="50" size="70" value="<%=desnome%>"  onBlur="aspa2(this)" onKeyUp="aspa(this)"> <%
      }
      else{
          rs = conexao.executaConsulta(query, session.getId() + "RS1");    
          if (rs.next()){       
        %> <input type="text" name="desnome" maxlength="50" size="70" value="<%=rs.getString(2)%>"  onBlur="aspa2(this)" onKeyUp="aspa(this)"> <%
      }
      }
      %> &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;
