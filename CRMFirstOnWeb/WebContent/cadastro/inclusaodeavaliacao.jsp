...
query = "SELECT AVA_CODIGO, AVA_DESCRICAO FROM AVALIACAO WHERE AVA_CODIGO = "+cod;
...
if(tipo.equals("I")){
  %>
  <%=trd.Traduz("INCLUSAO DE AVALIACAO")%></title>
  <%
}
else{
  %>
  <%=trd.Traduz("ALTERACAO DE AAVALIACAO")%></title>
  <%
}
%>
...
function envia(){
  if(document.frm.avanome.value==""){
    alert(<%=("\""+trd.Traduz("DIGITE A AVALIACAO")+"\"")%>);
    return false;
  }
  else{
    frm.action="avaliacaograva.jsp";
    frm.submit();
    return false; 
  }
}
          ...
          <td width="100%" align="center"> 
            <table border="0" cellspacing="0" cellpadding="0">
              <tr> 
                ...
                if(tipo.equals("I")){
                  %>
                  <%=trd.Traduz("INCLUSAO DE AVALIACAO")%>
      <%
    }
    else{
      %>
      <%=trd.Traduz("ALTERACAO DE AVALIACAO")%>
      <%
    }
    %>
    </td>
      ...
      <FORM name = "frm" method="POST">
          <td valign="top"> <img src="../art/bit.gif" width="159" height="1">&nbsp;<br><center>
                <table border="0" cellspacing="2" cellpadding="3" width="90%">
                  <tr class="celnortab"> 
                    <td>        
                     ...
                     if (tipo.equals("I")){ 
                      %> 
                        <input type="text" name="avanome" maxlength="50" size="70" onKeyUp="aspa(this)" onBlur="aspa2(this)">
                      <%
                     }
                     else{
             ...
             if(rs.next()) {
                      %> 
                        <input type="text" name="avanome" maxlength="50" size="70" value="<%=rs.getString(2)%>" onKeyUp="aspa(this)" onBlur="aspa2(this)">
                      <%}
                     }  
                     %>
                     <input type="hidden" name="tipo" value="<%=tipo%>">
         <input type="hidden" name="rad" value="<%=cod%>">
                    </td>
                  </tr>
                  <tr> 
                    <td align="center">&nbsp;<br>
                      <input type="button" onClick="return envia();"  value="        <%=trd.Traduz("OK")%>        " class="botcin"> 
                      &nbsp; <input type="button" onClick="return cancela()"  value=<%=("\""+trd.Traduz("CANCELAR")+"\"")%> class="botcin"></td>
                  </tr>
                </table>
                <p>&nbsp;
              </center>
