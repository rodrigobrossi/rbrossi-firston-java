...
<head>
<title>FirstOn - <%=trd.Traduz("AVALIACAO")%> - <%=trd.Traduz("MONTAR QUESTIONARIO")%></title>
<script language="JavaScript" src="/js/scripts.js"></script>

<script language="JavaScript">
function filtra(){
  //document.frm.tipo.value = "F";
  frm.action ="questionario.jsp";
  frm.submit();
  return false; 
}
function inclui(){
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<table border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="13"><img src="../art/bit.gif" width="13"
							height="15"></td>
						<td class="trontrk" align="center"><%=trd.Traduz("MONTAR QUESTIONARIO")%></td>
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td height="20" class="ctfontc" align="center">&nbsp; <%=trd.Traduz("LOCALIZAR POR QUESTIONARIO")%>:
						<%
 ...
 %> <input type="text" name="filtro" value="<%=valorFiltro%>"
					...
					<%
						if (!valorFiltro.equals(""))
							valorFiltro = " WHERE QUE_NOME >= '" + valorFiltro + "'";

						query = "SELECT QUE_CODIGO, QUE_NOME FROM QUESTIONARIO "
								+ valorFiltro + " ORDER BY QUE_NOME";
					...
					<tr>
						...
						<table border="0" cellspacing="1" cellpadding="2" width="80%">
							<tr>
								...
								%>
								<td width="4%">&nbsp;</td>
								<td width="96%" class="celtittab"><%=trd.Traduz("QUESTIONARIOS")%>
								</td>
							</tr>
							<%
								String ok = "";
							...
							<tr class="celnortab">
								...
								<td width="96%"><a
									href="inclusaodequestionario2.jsp?cod=<%=rs.getString(1)%>"
									class="lnk"><%=rs.getString(2)%></a></td>
							</tr>
							<%
								m++;
									} while (rs.next());
							...
							%>
							<tr>
								<td class="celtittab" align="center" colspan="100%"><%=trd.Traduz("NENHUM REGISTRO ENCONTRADO")%>
								...</td>
							</tr>
							<%
								}
