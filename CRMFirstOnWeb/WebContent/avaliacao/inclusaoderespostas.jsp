...
<%
	...
	ResultSet rs;
	String query = "SELECT RES_CODIGO, RES_NOME FROM RESPOSTA "
...
if (tipo.equals("I")) {
%> <%=trd.Traduz("INCLUSAO DE RESPOSTAS")%> <%
 } else {
 %> <%=trd.Traduz("ALTERACAO DE RESPOSTAS")%> <%
 }
 %>
</title>
...
function envia(){
  if(document.frm.res_nome.value==""){
    alert(<%=("\""+trd.Traduz("DIGITE UMA RESPOSTA")+"\"")%>);
    return false;
  }
  else{
    frm.action = "respostagrava.jsp";
    frm.submit();
    return false; 
  }
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	...
	<tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			if (ponto.equals("..")) {
			%>
			<jsp:include page="../menu/menu_avaliacao.jsp"
				flush="true">
 ...
 %>
			...
			<tr>
				...
				<table border="0" cellspacing="2" cellpadding="3" width="70%">
					<tr class="celnortab">
						<td>
						<p><%=trd.Traduz("RESPOSTA")%><br>
						<%
						if (tipo.equals("I")) {
						%> <input type="text" name="res_nome"
							maxlength="50" size="70" onKeyUp="aspa(this)"
							onBlur="aspa2(this)"> <%
 } else {
 %> <input type="text" name="res_nome"
							maxlength="50" size="70" value="<%=nome_res%>"
							onKeyUp="aspa(this)" onBlur="aspa2(this)"> <%
 }
 %> <input type="hidden" name="tipo"
							value="<%=tipo%>"> <input type="hidden" name="cod"
							value="<%=cod%>">
						</td>
					</tr>
					<tr>
