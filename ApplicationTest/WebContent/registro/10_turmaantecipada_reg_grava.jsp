...
String usu_plano = ""; usu_plano = usu_plano.valueOf(session.getAttribute("usu_plano"));
String turma = (request.getParameter("turma")== null)?"":(String)request.getParameter("turma");
...
query = "UPDATE TREINAMENTO SET TUR_CODIGO_REAL = " + turma + " WHERE TUR_CODIGO_PLAN_ANT = " + turma;
//out.println(query);
conexao.executaAlteracao(query);
    ...
    if (!request.getParameter("chk_"+i).equals("")) {
      ...
      if (!rs.next()) {//senao foi cadastrado, cadastra!
        query = "INSERT INTO PROCESSO (FUN_CODIGO_RESP, QUE_CODIGO, PRO_ENVLAUDO, PRO_FIM, TUR_CODIGO) "+
                "VALUES (" +usu_codigo+ ", " +questionario+ ", CONVERT(datetime,'"+envio+"',103), "+
        ...
        rs_P = conexao.executaConsulta(query,session.getId()+"RS_1");
        if (rs_P.next()){
...
//insere avaliados
query = "SELECT FUN_CODIGO FROM TREINAMENTO WHERE TUR_CODIGO_PLAN_ANT = " +turma;
...
if (rs_F.next()) {
  do {
    for (int ii=0; ii < processos.size(); ii++) {
      query = "INSERT INTO AVALIADO (PRO_CODIGO, FUN_CODIGO) VALUES (" +processos.elementAt(ii)+ ", " +rs_F.getString(1)+ ")";
      conexao.executaAlteracao(query);

      query = "SELECT FUN_CODSOLIC FROM FUNCIONARIO WHERE FUN_CODIGO = "+rs_F.getString(1);
      ResultSet rsU = conexao.executaConsulta(query,session.getId()+"RS_5");
      rsU.next();
      ...
      ResultSet rsV = conexao.executaConsulta(query,session.getId()+"RS_4");
      rsV.next();
            query = "INSERT INTO AVALIADOR (PRO_CODIGO, AVD_CODIGO, FUN_CODIGO) "+
              "VALUES ("+processos.elementAt(ii)+", "+rsV.getString(1)+", "+rsU.getString(1)+")";
      conexao.executaAlteracao(query);
      if(rsU!=null){
        rsU.close();
