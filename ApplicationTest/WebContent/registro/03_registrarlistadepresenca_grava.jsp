...
<%
	try {
		...
		String usu_codigo = "";
		usu_codigo = (session.getAttribute("fun_codigo") == null) ? ""
				: usu_codigo.valueOf((Integer) session
						.getAttribute("fun_codigo"));
		String usu_tipo = (session.getAttribute("usu_tipo") == null) ? ""
				: (String) session.getAttribute("usu_tipo");
		String usu_nome = (session.getAttribute("usu_nome") == null) ? ""
				: (String) session.getAttribute("usu_nome");
		String usu_login = (session.getAttribute("usu_login") == null) ? ""
				: (String) session.getAttribute("usu_login");
		Integer usu_fil = (Integer) session.getAttribute("usu_fil");
		Integer usu_idi = (Integer) session.getAttribute("usu_idi");
		String usu_plano = "";
		usu_plano = (session.getAttribute("usu_plano") == null) ? ""
				: usu_plano.valueOf(session.getAttribute("usu_plano"));
		int pag = (((String) session.getAttribute("pagina") == null) ? 7
		...
		String cuscur = (request.getParameter("txtcustocurso") == null) ? ""
				: request.getParameter("txtcustocurso");
		cuscur = replaceString(cuscur, ",", ".");
		String cuslog = (request.getParameter("txtcustolog") == null) ? ""
				: request.getParameter("txtcustolog");
		cuslog = replaceString(cuslog, ",", ".");
		String duracao = (request.getParameter("txtduracao") == null) ? ""
				: request.getParameter("txtduracao");
		String duracao2 = (request.getParameter("txtduracao2") == null) ? ""
				: request.getParameter("txtduracao2");
		...
		String obs = (request.getParameter("txtobs") == null) ? ""
				: request.getParameter("txtobs");
		int cont_avaliacao = (request.getParameter("cont_avaliacao") == null) ? 0
		...
		//versao
		query = "SELECT CUR_VERSAOATUAL FROM CURSO WHERE CUR_CODIGO = "
				+ cur;
		...
		//Para os Planejados
		queryt = "SELECT MAX(TUR_CODIGO) FROM TURMA WHERE FUN_CODIGO = "
				+ usu_codigo
				+ " AND CUR_CODIGO = "
				+ cur
		...
		for (int k = 0; k < funcvet.size(); k++) {
			...
			query = "UPDATE TREINAMENTO SET TUR_CODIGO_REAL = " + turma
					+ ", TTR_CODIGO = 1 WHERE TEF_CODIGO = " + strpa[0];
			conexao.executaAlteracao(query);
		}
		if (rs != null) {
			rs.close();
		...
		for (int jj = 0; jj < funcvetnp.size(); jj++) {
			...
			querynp = "SELECT TEF_CODIGO FROM TREINAMENTO WHERE FUN_CODIGO = "
					+ strnpa[1]
					+ " AND CUR_CODIGO = "
					+ cur
					+ " AND TEF_DATASOLICITACAO = DATEFMT("
					+ dia
					+ ") AND TEF_RESULTADOESPERADO = '" + obs + "'";
			//out.println("querynp = " + querynp);
			rsnp = conexao.executaConsulta(querynp, session.getId()
					+ "RS_3");
			if (rsnp.next()) {
				query = "UPDATE TREINAMENTO SET TUR_CODIGO_REAL = "
						+ turma
						+ ", TTR_CODIGO = 1 WHERE TEF_CODIGO = "
						+ rsnp.getString(1);
			...
			if (request.getParameter("chk_" + i) != null) {
				if (!request.getParameter("chk_" + i).equals("")) {
					...
					questionario = request
							.getParameter("cbo_questionario_" + i);
					envio = request.getParameter("txt_dt_envio_" + i);
					vencimento = request
							.getParameter("txt_dt_vencimento_" + i);

					//Planejados
					for (int ii = 0; ii < funcvet.size(); ii++) {
						String[] strpa = (String[]) funcvet
								.elementAt(ii);
						query = "SELECT FUN_CODIGO FROM TREINAMENTO WHERE TEF_CODIGO = "
								+ strpa[0];
						rsF = conexao.executaConsulta(query, session
								.getId()
								+ "RS_4");
						...
						if (rs.getString(1) != null) {
							...
							rsa = conexao.executaConsulta(query,
									session.getId() + "RS_6");
					...
					for (int k = 0; k < funcvetnp.size(); k++) {
						...
						String[] strnpa = (String[]) funcvetnp
								.elementAt(k);
...
<form name="frm" method="post"><input type="hidden" name="solic"
	value="<%=s_solic%>"></form>
</body>
</html>
<%
	//limpa vetores
		...
		query = "DELETE FROM INCNPTEMP WHERE INC_USU_CODIGO = "
				+ usu_codigo;
		...
		if (s_solic > 0) {
			...
			out
					.print("		alert(\" "
							+ trd.Traduz("NAO FOI POSSIVEL INSERIR")
							+ "+frm.solic.value+ \"PARTICIPANTE(S) NA AVALIACAO. FUNCIONARIO(S) SEM SOLICITANTE \" )");
			out
					.print("                window.open(\"frame_principal.jsp\",\"_self\");");
			out.print("</script>");
		...
		out.print("<script language=\"JavaScript\">");
		out.print("	    alert( \" "
				+ trd.Traduz("REGISTRO REALIZADO COM SUCESSO")
				+ " \" ! )");
