...
if(!(pag_dt_inicio.equals(""))) {
    ...
    query_dt = "AND U.TUR_DATAINICIO >= CONVERT(DATETIME, '"+data_inicial+"', 103) ";
    query_dt_ant = "AND T.TEF_INICIO >= CONVERT(DATETIME, '"+data_inicial+"', 103) ";
    filtros = filtros + "<BR>" + trd.Traduz("DATA INICIAL") + ": " + data_inicial;
}

if(!(pag_dt_fim.equals(""))) {
    data_final = pag_dt_fim;
    query_dt = query_dt + "AND U.TUR_DATAFINAL <= CONVERT(DATETIME, '"+data_final+"', 103) ";
    query_dt_ant = query_dt_ant + "AND T.TEF_FIM <= CONVERT(DATETIME, '"+data_final+"', 103) ";
