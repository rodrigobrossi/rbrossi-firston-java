...
import br.com.crm.control.RelacionamentoCTR;
import br.com.crm.db.CrmRelacionamento;
 ...
 *
 */
public class RelacionamentoResults extends AncestorFrame {
	...
	private RelacionamentoTableModel model;

	public RelacionamentoResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new RelacionamentoTableModel());
...
class RelacionamentoTableModel extends DefaultTableModel {
	...
	public RelacionamentoTableModel() {
		loadModel();
...
RelacionamentoCTR ctr = new RelacionamentoCTR();
		ctr.setObject(new CrmRelacionamento());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmRelacionamento element = (CrmRelacionamento) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmRelacionamento) model.get(column)).getId();

		case 1:
			return ((CrmRelacionamento) model.get(column)).getMerNaturalidade();

		case 2:
			return ((CrmRelacionamento) model.get(column)).getMerProfissao();
		case 3:
