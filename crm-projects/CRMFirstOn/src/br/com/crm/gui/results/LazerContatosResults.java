 ...
 * 
 */
public class LazerContatosResults extends AncestorFrame {
	...
	private LazerContatosTableModel model;

	private static final Logger logger = LoggerFactory.getLogger(LazerContatosResults.class);

	public LazerContatosResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new LazerContatosTableModel());
	...
	private static Component createHeader(String title, int type) {
		...
		try {
			BufferedImage iconImage = ImageIO.read(CRMComponentsUtils.class.getResource(SEARCH_IMAGE.replace(".ico", ".png")));
			lable.setIcon(new ImageIcon(iconImage));
		} catch (IOException e) {
			logger.log("Nao foi possivel carregar o icone: " + SEARCH_IMAGE);
		}
...
class LazerContatosTableModel extends DefaultTableModel {
	...
	public LazerContatosTableModel() {
		loadModel();
	...
	public void loadModel() {
		...
		LazerContatosCTR ctr = new LazerContatosCTR();
		ctr.setObject(new CrmLazerCliente());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmLazerCliente element = (CrmLazerCliente) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmLazerCliente) model.get(column)).getId();

		case 1:
			return ((CrmLazerCliente) model.get(column)).getLazTitulo();

		case 2:
			return ((CrmLazerCliente) model.get(column)).getLazObservacao();
		case 3:
