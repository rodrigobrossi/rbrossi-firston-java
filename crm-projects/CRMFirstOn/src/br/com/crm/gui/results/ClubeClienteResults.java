 ...
 *
 */
public class ClubeClienteResults extends AncestorFrame {
	...
	private ClubeClienteTabletModel model;

	public ClubeClienteResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new ClubeClienteTabletModel());
	...
	private static Component createHeader(String title, int type) {
		...
		// Exemplo: BitmapDescriptor bitmapDescriptor = descriptor.getDescriptor(0);
...
class ClubeClienteTabletModel extends DefaultTableModel {
	...
	public ClubeClienteTabletModel() {
		loadModel();
	...
	public void loadModel() {
		...
		ClubeClienteCRT ctr = new ClubeClienteCRT();
		ctr.setObject(new CrmClubeCliente());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmClubeCliente element = (CrmClubeCliente) it.next();
			boolean add = model.add(element);
			if (!add) {
				System.out.println("[Model]: could not read :"
						+ element.getCclTitulo());
			}
		}
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmClubeCliente) model.get(column)).getId();

		case 1:
			return ((CrmClubeCliente) model.get(column)).getCclTitulo();

		case 2:
			return ((CrmClubeCliente) model.get(column)).getCclObservacao();
		case 3:
