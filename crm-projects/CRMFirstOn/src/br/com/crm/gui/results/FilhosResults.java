 ...
 * 
 */
public class FilhosResults extends AncestorFrame {
	...
	private FilhosTableModel model;

	private static final Logger logger = LoggerFactory.getLogger(FilhosResults.class);

	public FilhosResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new FilhosTableModel());
...
class FilhosTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		FilhosCTR ctr = new FilhosCTR();
		ctr.setObject(new CrmFilhos());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmFilhos element = (CrmFilhos) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmFilhos) model.get(column)).getId();

		case 1:
			return ((CrmFilhos) model.get(column)).getFilNome();

		case 2:
			return ((CrmFilhos) model.get(column)).getFilIdade();
		case 3:
