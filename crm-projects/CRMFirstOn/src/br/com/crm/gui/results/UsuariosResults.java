 ...
 * 
 */
public class UsuariosResults extends AncestorFrame {
	...
	public void createGUI() {
		...
		table = new JTable(model = new UsuariosTableModel());
...
class UsuariosTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		UsuariosCTR ctr = new UsuariosCTR();
		ctr.setObject(new CrmUsuarios());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmUsuarios element = (CrmUsuarios) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmUsuarios) model.get(column)).getId();

		case 1:
			return ((CrmUsuarios) model.get(column)).getUsrNomeUsuario();

		case 2:
			return ((CrmUsuarios) model.get(column)).getUsrDataAdmissao();
		case 3:
