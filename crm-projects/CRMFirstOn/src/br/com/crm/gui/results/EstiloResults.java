 ...
 * 
 */
public class EstiloResults extends AncestorFrame {
	...
	private EstiloTableModel model;

	public EstiloResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new EstiloTableModel());
...
class EstiloTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		EstiloClienteCTR ctr = new EstiloClienteCTR();
		ctr.setObject(new CrmEstiloCliente());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmEstiloCliente element = (CrmEstiloCliente) it.next();
			boolean add = model.add(element);
			if (!add) {
				System.out.println("[Model]: could not read :"
						+ element.getEstTitulo());
			}
		}
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmEstiloCliente) model.get(column)).getId();

		case 1:
			return ((CrmEstiloCliente) model.get(column)).getEstTitulo();

		case 2:
			return ((CrmEstiloCliente) model.get(column)).getEstOservacao();
		case 3:
