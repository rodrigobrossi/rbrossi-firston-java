...
import br.com.crm.control.RegistroVisitaCTR;
import br.com.crm.db.CrmRegistroVisita;
 ...
 *
 */
public class RegistroVisitaResults extends AncestorFrame {
	...
	private RegistroVisitaTableModel model;

	public RegistroVisitaResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new RegistroVisitaTableModel());
...
class RegistroVisitaTableModel extends DefaultTableModel {
	...
	public RegistroVisitaTableModel() {
		loadModel();
...
RegistroVisitaCTR ctr = new RegistroVisitaCTR();
		ctr.setObject(new CrmRegistroVisita());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmRegistroVisita element = (CrmRegistroVisita) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmRegistroVisita) model.get(column)).getId();

		case 1:
			return ((CrmRegistroVisita) model.get(column)).getRgvMotivo();

		case 2:
			return ((CrmRegistroVisita) model.get(column)).getRgvDataVisita();
		case 3:
