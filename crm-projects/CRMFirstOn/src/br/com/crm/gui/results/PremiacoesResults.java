...
import br.com.crm.control.PremiacoesCTR;
import br.com.crm.db.CrmPremiacoes;
 ...
 *
 */
public class PremiacoesResults extends AncestorFrame {
	...
	private PremiassoesTableModel model;

	public PremiacoesResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new PremiassoesTableModel());
...
class PremiassoesTableModel extends DefaultTableModel {
...
PremiacoesCTR ctr = new PremiacoesCTR();
		ctr.setObject(new CrmPremiacoes());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmPremiacoes element = (CrmPremiacoes) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmPremiacoes) model.get(column)).getId();

		case 1:
			return ((CrmPremiacoes) model.get(column)).getPrmDescricao();

		case 2:
			return ((CrmPremiacoes) model.get(column)).getPrmFormaPagamento();
		case 3:
