...
import br.com.crm.control.NetosCTR;
import br.com.crm.db.CrmNetos;
 ...
 * 
 */
public class NetosResults extends AncestorFrame {
	...
	private NetosTableModel model;

	private static final Logger logger = LoggerFactory.getLogger(NetosResults.class);
	...
	public void createGUI() {
		...
		table = new JTable(model = new NetosTableModel());
...
class NetosTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		NetosCTR ctr = new NetosCTR();
		ctr.setObject(new CrmNetos());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmNetos element = (CrmNetos) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmNetos) model.get(column)).getId();

		case 1:
			return ((CrmNetos) model.get(column)).getNetNome();

		case 2:
			return ((CrmNetos) model.get(column)).getNetIdade();
		case 3:
