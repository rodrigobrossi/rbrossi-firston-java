...
import br.com.crm.control.RamoAtividadeCTR;
import br.com.crm.db.CrmRamoAtividade;
 ...
 *
 */
public class RamoAtividadesResults extends AncestorFrame {
	...
	private RamoAtividadesTableModel model;

	public RamoAtividadesResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new RamoAtividadesTableModel());
...
class RamoAtividadesTableModel extends DefaultTableModel {
	...
	public RamoAtividadesTableModel() {
		loadModel();
...
RamoAtividadeCTR ctr = new RamoAtividadeCTR();
		ctr.setObject(new CrmRamoAtividade());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmRamoAtividade element = (CrmRamoAtividade) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmRamoAtividade) model.get(column)).getId();

		case 1:
			return ((CrmRamoAtividade) model.get(column)).getRatTitulo();

		case 2:
			return ((CrmRamoAtividade) model.get(column)).getRatObservacao();
		case 3:
