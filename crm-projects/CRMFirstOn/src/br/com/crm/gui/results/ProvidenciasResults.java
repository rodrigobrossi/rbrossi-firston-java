...
import br.com.crm.control.ProvidenciasCTR;
import br.com.crm.db.CrmProvidencias;
 ...
 *
 */
public class ProvidenciasResults extends AncestorFrame {
	...
	private ProvidenciasTableModel model;

	public ProvidenciasResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new ProvidenciasTableModel());
...
class ProvidenciasTableModel extends DefaultTableModel {
...
ProvidenciasCTR ctr = new ProvidenciasCTR();
		ctr.setObject(new CrmProvidencias());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmProvidencias element = (CrmProvidencias) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmProvidencias) model.get(column)).getId();

		case 1:
			return ((CrmProvidencias) model.get(column)).getPrvParaQuando();

		case 2:
			return ((CrmProvidencias) model.get(column)).getPrvQuemDeveProvidenciar();
		case 3:
