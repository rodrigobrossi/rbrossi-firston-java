...
import br.com.crm.control.HistoricoCotatosCTR;
import br.com.crm.db.CrmHistoricoContato;
 ...
 *
 */
public class HistoricoContatosResults extends AncestorFrame {
	...
	private HistoricoContatosTableModel model;

	public HistoricoContatosResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new HistoricoContatosTableModel());
	...
	private static Component createHeader(String title, int type) {
		...
		try {
			// Exemplo: icoFile = new ICOFile(CRMComponentsUtils.class.getResource(SEARCH_IMAGE));
		} catch (IOException e) {
...
class HistoricoContatosTableModel extends DefaultTableModel {
	...
	public HistoricoContatosTableModel() {
		loadModel();
	...
	public void loadModel() {
		...
		HistoricoCotatosCTR ctr = new HistoricoCotatosCTR();
		ctr.setObject(new CrmHistoricoContato());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmHistoricoContato element = (CrmHistoricoContato) it.next();
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmHistoricoContato) model.get(column)).getId();

		case 1:
			return ((CrmHistoricoContato) model.get(column)).getHisData();

		case 2:
			return ((CrmHistoricoContato) model.get(column)).getHisResumo();
		case 3:
