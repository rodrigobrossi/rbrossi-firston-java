...
import br.com.crm.control.DatasComemorativasCTR;
import br.com.crm.db.CrmDatasComemorativas;
 ...
 * 
 */
public class DatasComemorativasResults extends AncestorFrame {
	...
	private DatasComemorativasTableModel model;

	public DatasComemorativasResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new DatasComemorativasTableModel());
	...
	private static Component createHeader(String title, int type) {
		...
		try {
			// Exemplo: icoFile = new ICOFile(CRMComponentsUtils.class
			// .getResource(SEARCH_IMAGE));
		} catch (IOException e) {
...
class DatasComemorativasTableModel extends DefaultTableModel {
	...
	public DatasComemorativasTableModel() {
		loadModel();
	...
	public void loadModel() {
		...
		DatasComemorativasCTR ctr = new DatasComemorativasCTR();
		ctr.setObject(new CrmDatasComemorativas());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmDatasComemorativas element = (CrmDatasComemorativas) it.next();
			boolean add = model.add(element);
			if (!add) {
				System.out.println("[Model]: could not read :"
						+ element.getDcmTitulo());
			}
		}
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmDatasComemorativas) model.get(column)).getId();

		case 1:
			return ((CrmDatasComemorativas) model.get(column)).getDcmClube();

		case 2:
			return ((CrmDatasComemorativas) model.get(column)).getDcmCrenca();
		case 3:
