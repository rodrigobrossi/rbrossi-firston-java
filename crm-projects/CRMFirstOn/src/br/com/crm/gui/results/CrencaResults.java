 ...
 *
 */
public class CrencaResults extends AncestorFrame {
	...
	private CrencaTableModel model;

	private static final Logger logger = LoggerFactory.getLogger(CrencaResults.class);

	public CrencaResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new CrencaTableModel());
	...
	private static Component createHeader(String title, int type) {
		...
		// lable.setIcon(new ImageIcon(icon.getImage()));
		// Placeholder: set icon to null
		lable.setIcon(null);
...
class CrencaTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		CrencaClienteCTR ctr = new CrencaClienteCTR();
		ctr.setObject(new CrmCrencaCliente());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmCrencaCliente element = (CrmCrencaCliente) it.next();
			boolean add = model.add(element);
			if (!add) {
				System.out.println("[Model]: could not read :"
						+ element.getCrcTitulo());
			}
		}
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmCrencaCliente) model.get(column)).getId();

		case 1:
			return ((CrmCrencaCliente) model.get(column)).getCrcTitulo();

		case 2:
			return ((CrmCrencaCliente) model.get(column)).getCrcObservacao();
		case 3:
