 ...
 * 
 */
public class EsporteResults extends AncestorFrame {
	...
	private EsportTableModel model;

	private static final Logger logger = LoggerFactory.getLogger(EsporteResults.class);

	public EsporteResults() {
		createGUI();
	...
	public void createGUI() {
		...
		table = new JTable(model = new EsportTableModel());
...
class EsportTableModel extends DefaultTableModel {
	...
	public void loadModel() {
		...
		EsporteClienteCTR ctr = new EsporteClienteCTR();
		ctr.setObject(new CrmEsporteCliente());
		Iterator it = ctr.getData().iterator();
		while (it.hasNext()) {
			CrmEsporteCliente element = (CrmEsporteCliente) it.next();
			boolean add = model.add(element);
			if (!add) {
				System.out.println("[Model]: could not read :"
						+ element.getEspTitulo());
			}
		}
	...
	public Object getValueAt(int column, int row) {
		...
		switch (row) {
		case 0:
			return ((CrmEsporteCliente) model.get(column)).getId();

		case 1:
			return ((CrmEsporteCliente) model.get(column)).getEspObservacao();

		case 2:
			return ((CrmEsporteCliente) model.get(column)).getEspTitulo();
		case 3:
