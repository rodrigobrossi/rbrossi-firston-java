...
String usu_nome 	= trataAspas((String)session.getAttribute("usu_nome")); 
String usu_login 	= trataAspas((String)session.getAttribute("usu_login")); 
...
String sara			= (request.getParameter("sel_saratoga")==null)?"NULL":request.getParameter("sel_saratoga");
String cur 			= (request.getParameter("curso")==null)?"":trataAspas(request.getParameter("curso"));
String tip 			= (request.getParameter("sel_tipo")==null)?"":request.getParameter("sel_tipo");
...
String cuscur 		= (request.getParameter("txtcustocurso")==null)?"":trataAspas(request.getParameter("txtcustocurso"));
String cuslog_txt 	= (String)request.getParameter("txtcustolog");
Float cuslog 		= Float.valueOf(cuslog_txt.replace(',','.'));
String duracao 		= (request.getParameter("txtduracao")==null)?"":trataAspas(request.getParameter("txtduracao"));
String duracao2 	= (request.getParameter("txtduracao2")==null)?"":trataAspas(request.getParameter("txtduracao2"));
...
String obs 			= (request.getParameter("txtobs")==null)?"":trataAspas(request.getParameter("txtobs"));
...
query = "INSERT INTO CURSO (DES_CODIGO, CUR_ATIVO, SAR_CODIGO, CUR_NOME, TIT_CODIGO, CUR_DURACAO, CUR_CUSTO, "+
        ...
        "VALUES (" +des+ ", 'N',"+sara+", '"+cur+"', "+tit+", "+dur+", "+cuscur+", "+tip+", "+
        "'SIMPLES', 'SIMPLES', 'SIMPLES', DATEFMT("+dia+"), "+
        "'SIMPLES', "+cuslog+", '1', 'SIMPLES', "+entidade+", "+ usu_codigo +", 'S')"; 
...
query = "SELECT CUR_CODIGO FROM CURSO " +
        "WHERE CUR_NOME = '" + cur + "' AND " +
        "CUR_DATACADASTRO = DATEFMT("+dia+") AND " +
        "TIT_CODIGO = " +tit+ " AND " +
...
if(!(instrutor.equals("")))
{
	query = "INSERT INTO TURMA (EMP_CODIGO, FUN_CODIGO, TUR_DATAINICIO, " + 
	        ...
	        "TUR_OBS, TUR_PLANEJADA, TUR_REPROGRAMADA, TUR_REGISTRADA, TUR_CUSTO2, TTR_CODIGO) VALUES (" + entidade + 	", " + usu_codigo + ", " +
		    "DATEFMT(" + dtini + "), DATEFMT(" + dtfim + "), " + usu_plano + ", " + 
			dur + ", " + instrutor + ", " + cuscur + ", " + codcur + ", 0, '" + obs + "', 'N', 'N', 'S', " + cuslog + ", 4)";
}
...
else
{
		query = "INSERT INTO TURMA (EMP_CODIGO, FUN_CODIGO, TUR_DATAINICIO, " + 
	        ...
	        "TUR_OBS, TUR_PLANEJADA, TUR_REPROGRAMADA, TUR_REGISTRADA, TUR_CUSTO2, TTR_CODIGO) VALUES (" + entidade + 	", " + usu_codigo + ", " +
		    "DATEFMT(" + dtini + "), DATEFMT(" + dtfim + "), " + usu_plano + ", " + 
			dur + ", " + cuscur + ", " + codcur + ", 0, '" + obs + "', 'N', 'N', 'S', " + cuslog + ", 4)";
}
...
//Para os Planejados
query = "SELECT (TUR_CODIGO) FROM TURMA WHERE FUN_CODIGO = " + usu_codigo + " " +
        "AND CUR_CODIGO = " + codcur + " AND TUR_DATAINICIO = DATEFMT(" + dtini + ") " +
        "AND TUR_OBS = '" +  obs + "' ";
//out.println("******"+query+"*******");
...
for(int k=0 ; k<funcvet.size(); k++) {
    query = "INSERT INTO TREINAMENTO (FUN_CODIGO, CUR_CODIGO, PLA_CODIGO, TTR_CODIGO, QBR_CODIGO, " +
            ...
            "VALUES (" + funcvet.elementAt(k) + ", " + codcur + ", " + usu_plano + ", 4" + ", 1, " + dur + ", " + cuscur + ", " + 
            "DATEFMT("+dia+"), '" + obs + "', 'N', 1, " + turma+")";
...
<%@page import="firston.eval.components.FOLocalizationBean"%><script language="JavaScript">
    alert(<%=("\""+trd.Traduz("REGISTRO REALIZADO COM SUCESSO!")+"\"")%>);
