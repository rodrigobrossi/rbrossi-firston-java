...
<head>
<title>FirstOn - <%=trd.Traduz("CADASTRO")%> - <%=trd.Traduz("CADASTRO DE FUNCIONARIOS")%> - <%=trd.Traduz("EFETIVACAO DE TERCEIRO")%></title>
...
<%
  query = "SELECT F.FUN_CODIGO, F.FUN_CHAPA, F.FUN_NOME, FI.FIL_NOME, F.FUN_CODSOLIC, "+
    ...
    "T3.TB3_CODIGO =* F.TB3_CODIGO AND "+
    "T2.TB2_CODIGO =* F.TB2_CODIGO AND "+
    "FI.FIL_CODIGO =* F.FIL_CODIGO AND "+
  ...
  if(rs.next()){
    ...
    <%
    String queryH = "SELECT FUN_NOME FROM FUNCIONARIO WHERE FUN_CODIGO = "+rs.getString(5);
...
<input type="hidden" name="codigo" value="<%=fun_cod%>">
<br><br><br>
<input type="button" value="   EFETIVAR   " class="botcin" OnClick="return efetiva();">
<input type="button" value="   CANCELAR   " class="botcin" OnClick="return cancela();">
</center>
</form>
    </td>
  </tr>
