...
function propertyTransfer() {
    ...
    for (temp in this.obj) {
	this[temp] = this.obj[temp]
    }
}
...
function HM_f_GetItemDivStr(itemid, disptext, hasmore) {
    var WidthValue = HM_CurrentMenu.isHorizontal ? (ItemElement.isLastItem) ? (HM_CurrentTree.MenuWidth
	    - HM_BorderWidth - HM_SeparatorSize)
	    : (HM_CurrentTree.MenuWidth - HM_BorderWidth)
	    : HM_CurrentTree.ItemWidth;
    var TempString = "<DIV ID=" + itemid + " STYLE='position:absolute;width:"
	    + WidthValue + "px'>";
	...
	var ImgPosition = HM_CurrentTree.RightToLeft ? "absolute;"
		: "relative;";
	...
	var ImgStyle = HM_CurrentTree.RightToLeft ? ("left:"
		+ (HM_ItemPadding + HM_ImageHorizSpace) + "px;top:"
		+ (HM_ItemPadding + HM_ImageVertSpace) + "px;")
		: ("float:right;margin-right:" + (-FullPadding)
			+ "px;margin-top:" + HM_ImageVertSpace + "px;width:"
			+ HM_ImageSize + "px;");
	var ImgString = "<IMG ID='HM_ImMore' STYLE='position:" + ImgPosition
		+ ImgStyle + "' SRC='" + ImgSrc + "' HSPACE=" + ImgHSpace
		+ " VSPACE=0 BORDER=0>";
...
function HM_f_MakeMenu(menucount) {
	...
	HM_CurrentMenu.htmlString = "<DIV ID='" + HM_MenuIDPrefix + menucount
		...
		+ HM_CurrentTree.MenuWidth + "'>";
    }

    while (HM_CurrentMenu.itemCount < HM_CurrentMenu.maxItems) {
	HM_CurrentMenu.itemCount++;

	HM_CurrentItem = document.all(HM_ItemIDPrefix + menucount + "_"
		+ HM_CurrentMenu.itemCount);
	if (!HM_CurrentItem) {
	    if (HM_StatusDisplayBuild)
    ...
    document.body.insertAdjacentHTML("BeforeEnd", HM_CurrentMenu.htmlString
	    + "</DIV>");
...
function HM_f_SetItemStyle() {
	...
	if (this.menu.isHorizontal) {
	    if (this.isLastItem)
		pixelWidth = (HM_CurrentTree.MenuWidth - HM_BorderWidth - HM_SeparatorSize);
	    else
		pixelWidth = (HM_CurrentTree.MenuWidth - HM_BorderWidth);
	    pixelTop = 0;
...
function HM_f_FixSize(makevis) {
	...
	for (i = 0; i < this.childNodes.length; i++) {
		...
		TempItem.style.pixelTop = TempItem.siblingBelow.style.pixelTop
			+ SiblingHeight;
	    } else
		TempItem.style.pixelTop = 0;
	}
	this.style.pixelHeight = TempItem.style.pixelTop
		+ TempItem.scrollHeight + (HM_BorderWidth * 2);
    }
    this.sizeFixed = true;
    if (makevis)
...
function HM_f_PopMenu() {
	...
	return true;
    if (HM_CurrentMenu.tree.ClickStart && event.type != "click")
	return true;
...
function HM_f_ShowChild() {
	...
	this.child.yPos = this.menu.style.pixelTop + this.menu.offsetHeight
		- (HM_BorderWidth);
    } else {
...
function HM_f_ItemOver() {
    ...
    if (event.srcElement.id == "HM_ImMore")
	return;
    if (this.hasRollover) {
...
function HM_f_ItemOut() {
    if (event.srcElement.contains(event.toElement)
	    || (event.fromElement.tagName == "IMG" && event.toElement
